...
if ( "`whoami`" == "narayanaswamyy2" ) then
    ...
    #       >& /dev/null
    source /data/NIMH_SSCC/narayanaswamyy2/miniconda3/etc/profile.d/conda.csh   \
           >& /dev/null
endif
...
set dir_test_full  = ${dir_inroot}/data_14_test_full

# supplementary directory (reference data, etc.)
set dir_suppl      = ${dir_inroot}/supplements

# set output directory **
set sdir_out = ${dir_test_full}
set lab_out  = "TEST_FULL_VNET"
...
setenv PYTHONPATH /usr/local/apps/afni/current-py3/linux_rocky_8

python lib_ml_test.py \
    -data_path '/data/NIMH_SSCC/narayanaswamyy2/demo_vnet/data_01_lowres/validation/' \
    -outdir "${sdir_out}/test_low_res_14May" \
    -checkpoint '/data/NIMH_SSCC/narayanaswamyy2/CNNouts/checkpoint' \
    -device 'cpu'  \
    -mask_available 1
